...
| `HiatusSpottedAlert` | `lib/mc/hiatus_alerts.py` | Per-UUID 24h cooldown marker for the "hiatus user is online" post in `#activity`. One row per post; the cooldown is an existence check over a 24h window (`filter(uuid=…, created_at__gte=cutoff).exists()`), made by `hiatus_alerts` itself — the watcher cogs never touch this table. |
