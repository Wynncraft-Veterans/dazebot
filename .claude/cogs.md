...
| [`server_watcher.py`](../cogs/activity/server_watcher.py) | 2-min poll of Returners, waitlisted and HIATUS-role accounts whose `lastJoin` is privacy-hidden. Reads the `/v3/player` `server` field; between-tick changes — and any strict increase across the stat snapshot — are treated as activity and bump `last_online`. HIATUS hits funnel into `lib/mc/hiatus_alerts.maybe_alert_hiatus`. Note the stat-delta branch re-fires on essentially every tick of active play, which is why the "welcome back" DM gates on a `login_edge` derived from `online_seen_at` rather than on the detection itself. |
| [`hiatus_watcher.py`](../cogs/activity/hiatus_watcher.py) | 30-second poll of the bulk `/v3/player?identifier=uuid` endpoint (one request, whole online roster) diffed against an in-memory `_prev_online` set to catch HIATUS-role players coming online. Covers the ~84% whose `onlineStatus` is *not* privacy-hidden; `server_watcher` covers the rest. Also stamps `MinecraftAccount.online_seen_at` for the whole online cohort each tick — `_prev_online` is empty on every start, so without that persisted heartbeat every restart would look like the entire cohort logging in at once. |

## `cogs/maintenance/` — data-integrity janitor

| Cog | Owns |
|---|---|
| [`janitor.py`](../cogs/maintenance/janitor.py) | Periodic reconciliation loop (`JANITOR_INTERVAL_MINUTES`, default 6h) + staff `/janitor run`. Five reconcilers, run **(F)→(A)→(D)→(B)→(H)**: **(F)** blocklisted users' linked members forced REGISTERED-only (blocklist *is* positive info — spec §2b); **(A)** the Flame self-heal, *strictly* — only linked members with **no** membership state at all get the baseline **granted** (additive; never a demotion). Members with any valid primary role, **including manually-assigned HIATUS/HONOURARY**, are left untouched; genuine multi-primary conflicts are left for manual review. **(D)** WAITLISTED ↔ Waitlist-DB sync — the DB table is the source of truth: an orphaned role with no row is removed, a row whose member lacks the role gets it (runs after (A) so a just-healed stateless member can be waitlisted). **(B)** leftover `LinkCode` cleanup — unrecoverable rows deleted; for a linked user, a stale row (user already has a valid state) is just deleted *without touching roles*, only a *stateless* user triggers an additive baseline re-enforce. **(H)** HIATUS holders who are actually in a guild → the transition they're owed (on the Returners roster → MEMBER, elsewhere → REGISTERED), closing the "nothing scans a guild no Returners member is in" hole. Runs **last** because it is the only reconciler that spends Wynncraft API budget: one GUILD-bucket roster call per tick for the whole cohort (deliberately *not* `refresh_mc_guild` per player, which would re-fetch the roster each time), plus at most `JANITOR_HIATUS_SWEEP_MAX` (default 15) background-tier `/v3/player` calls, taken as a rotating slice so a cohort of C is fully covered every `ceil(C/max)` ticks. Log-only by default (`JANITOR_REPAIR_ENABLED=False`); flips to actually mutate at runtime. **Either way** it posts a throttled (`JANITOR_ALERT_DELTA`) summary embed to `JANITOR_ALERT_CHANNEL`, throttle bypassed when a tick actually repaired. `JANITOR_ENABLED`/interval are read at init/decoration → restart to change; `JANITOR_REPAIR_ENABLED` is the live switch. Throttle marker = `JanitorAlert` ORM table (mirror of `DeadGuildAlert`). |
