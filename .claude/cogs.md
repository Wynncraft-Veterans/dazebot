...
| [`server_watcher.py`](../cogs/server_watcher.py) | 3-min poll of Returners members whose `lastJoin` is privacy-hidden. Reads the `/v3/player` `server` field; between-tick changes are treated as activity and bump `last_online`, clearing them out of `/purgelist`'s Unknown bucket. |
...
| [`vetsmod.py`](../cogs/vetsmod.py) | `/vetsmod` (self-issue) + `/change remove key <target>` and `/change rotate key <target>` (staff). DMs the modrinth link + `/unlock <key>`. Falls back to `LINK_FALLBACK_CHANNEL` ping if DMs are closed (`VetsmodFallbackView`). Staff actions DM the affected user (with optional reason). See [verify_keys.md](verify_keys.md). |
