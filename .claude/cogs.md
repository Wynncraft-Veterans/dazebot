...
| [`waitlist.py`](../cogs/membership/waitlist.py) | `/waitlist add\|view\|remove\|self\|leave`. The `WAITLISTED` role is kept in lock-step with the `Waitlist` DB table: `add`/`self` apply the role (and `add` reports success if the role was already present — DB+role re-synced), `remove`/`leave` strip it via `apply_transition(INACTIVE_WAITLIST)`. The Waitlist table is the source of truth; janitor reconciler (D) heals any residual divergence. |
