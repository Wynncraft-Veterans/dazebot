...
- `cogs/maintenance/janitor.py` reconciler (A) — every linked member, every janitor interval (the Flame self-heal). Calls `_enforce_linked_baseline_for` in enforce mode; in log-only mode it mirrors the delta math to *detect* without mutating. Gated by `JANITOR_REPAIR_ENABLED` but detects+alerts even when repair is off.
