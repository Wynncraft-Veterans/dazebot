...
If `_enforce_linked_baseline_for` returns `False`, `try_consume_code` **keeps the `LinkCode` row** and logs an `ERROR`. The data-integrity janitor (`cogs/maintenance/janitor.py` reconciler B) now periodically reconciles kept rows: delete when unrecoverable (MC linked to a different `DiscordAccount`, or the Discord user is in no guild). For a row whose user IS linked it distinguishes two cases — **if the user already holds a valid primary state the row is just a stale leftover → deleted *without touching roles*** (so a legitimately manual HIATUS/HONOURARY is never demoted); **only a *stateless* user** is treated as a real kept-on-failure → an *additive* baseline re-enforce (nothing to strip) then the code is cleared. Re-attempt cadence = the janitor interval (default 6h) and only mutates when `JANITOR_REPAIR_ENABLED` is on (log-only otherwise, but still alerts). A player code re-send (re-invokes `try_consume_code`) or a staff `/link set` / `/janitor run` still recover it immediately. So keeping the row prevents *premature* `LinkCode` deletion **and** is now eventually reconciled rather than relying solely on manual re-trigger.
