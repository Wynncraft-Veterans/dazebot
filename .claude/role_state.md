...
- **The DM only goes to people who were actually away.** `HIATUS_RETURN_DM_MIN_AWAY_DAYS` (9, mirroring `/purgelist`'s default window — the criterion staff actually kick on) is checked against `MinecraftAccount.last_online`. That column works here *because* nothing refreshes it for a guildless account: `activity._apply_guild` only runs for members of a scanned roster, `server_watcher` only polls the privacy-hidden, and `hiatus_watcher` writes `online_seen_at` and never this. So at gate time it still holds their last real `lastJoin`, and `now - last_online` is exactly "how long they were gone". The one caller that breaks that — `server_watcher`, which bumps `last_online` on its way in — passes its pre-tick reading as `away_since`. An unknowable answer (the epoch sentinel) suppresses rather than sends; measured against the 2026-07-27 snapshot that costs 5 of 249 guildless accounts permanently, against 6 of 249 correctly blocked for being back too recently.
- **The DM's gates run in two phases, and the split is a cost decision.** `plan_return_dm` is pure reads and runs *before* the shared `refresh_mc_guild`, so a "no" there costs nothing; `verify_return_dm` runs *after* it and immediately before the send. The reason the second phase exists at all: the shared crosscheck only refreshes the **spotted** account, so the person's other linked accounts were judged on their stored `guild` — the exact column this cohort is known to rot. `verify_return_dm` checks those against the live API, and when one turns out to be in a guild it fires `heal_stale_hiatus` for *that* account, so the person gets the REGISTERED transition they were owed rather than just losing a DM. It **fails closed**: `refresh_mc_guild` leaves the stored value untouched on an API failure, and since every account reaching that point is stored as guildless by construction, "upstream down" and "confirmed guildless" would otherwise be the same observation — so it uses `lib/mc/resolve.live_guilds_for`, which returns None rather than a half-answer, and a None drops the DM. That helper also holds the repo's standing 1+N shape (one roster fetch for the whole set, background-tier player calls for the remainder) instead of the 2N that looping `refresh_mc_guild` would cost. Zero requests for anyone without alts.
