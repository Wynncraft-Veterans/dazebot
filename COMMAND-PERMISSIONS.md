...
| `/alerts status \| mute \| unmute \| thresholds` | STAFF | Shortcut wrappers around `/config` for guild dead/full alerts |
| `/alerts hiatus_mute \| hiatus_unmute` | STAFF | Master switch for hiatus-spotted detection — silences the `#activity` post **and** the return DM, and suspends the bulk `/v3/player` poll |
| `/alerts return_dm_on \| return_dm_off` | STAFF | Just the "welcome back from your hiatus" DM; leaves the `#activity` post running |
| `/alerts return_dm_check \| return_dm_clear <user>` | STAFF | Inspect / lift one user's return-DM mute or snooze |
