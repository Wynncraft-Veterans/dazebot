...
### `rotate_key(member)` — the `/vetsmod rotate` slash handler
...
### `revoke_key(disc_uuid, *, reason=None)` — the `/vetsmod revoke` staff handler
...
- **Tier is re-resolved on every call** against the live Discord member. Promote a user to MEMBER and they pick it up on the next WS reconnect (or whenever temporary-server's 60s LRU cache expires) — no `/vetsmod rotate` needed.
...
- **Don't add a `created_by` column expecting it to mean "the user who issued the key".** A staff `/vetsmod revoke @user` revokes someone else's key — the row is always owned by the target's `disc_uuid`.
